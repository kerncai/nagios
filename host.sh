...
/usr/bin/printf "$HOSTADDRESS\nInfo:$HOSTOUTPUT\npost time `date |awk '{print$4}'`" | /usr/bin/sendEmail -f idc10@mon10-003.i.ajkdns.com -t $EMAIL -cc $CONTACTEMAIL -s 1.1.1.1 -u "$NOTIFICATIONTYPE $HOSTNAME is $HOSTSTATE"
