...
#OWNER=`mysql  -h10.10.8.35 ops_db -e"select a.label,b.user_name from item a ,user b where a.owner_id=b.id and label ='$HOSTNAME'"|grep -v "label"|awk '{print$2}'`

#USER=`mysql -h10.10.8.35 ops_db -e"select a.label,b.chinese_name from item a ,user b where a.user_id=b.id and label ='$HOSTNAME'"|grep -v "label"|awk '{print$2}'`


#echo mysql -h10.10.8.35 ops_db -e"select a.label,b.email from item a ,user b where a.owner_id=b.id and label ='$HOSTNAME'" >>/tmp/myhostname
...
/usr/bin/sendEmail -o message-header=GB2312 -f ops_server@mon10-003.i.ajkdns.com -t $EMAIL -cc $CONTACTEMAIL -s 1.1.1.1 -u "IDC10:$HOSTALIAS/$SERVICEDESC is $SERVICESTATE" -m "Address:$HOSTADDRESS\nStatus:\n$SERVICEOUTPUT\nOwner:$OWNER\nUser:$USER\npost time `date|awk '{print $4}'`"
